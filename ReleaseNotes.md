...
In addition, we have updated the project to be netcore (semantically, in practice it is just called .NET) compatible. .NET Core is Microsoft's cross platform, open source platform
for its libraries. This also allows us to move Orchid forward instead of relying on .NET Framework. Everything should behave
...
if not, you can find find .NET 8 here (https://dotnet.microsoft.com/en-us/download). The latest version of .NET 6 (6.0.26) should be
