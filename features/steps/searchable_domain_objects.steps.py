...
def step_impl(context, name, surrounding):
    ...
    with warnings.catch_warnings(record=False):
        ...
        context.queried_domain_objects = list(context.project.data_frames().find_by_name(query_name))
...
def step_impl(context, display_name, surrounding):
    ...
    if surrounding == 'no':
        ...
        query_name = ' ' * random.choice(range(1, 3 + 1)) + display_name
    elif surrounding == 'trailing':
        query_name = display_name + ' ' * random.choice(range(1, 3 + 1))
    elif surrounding == 'leading and trailing':
        query_name = ' ' * random.choice(range(1, 3 + 1)) + display_name + ' ' * random.choice(range(1, 3 + 1))
    else:
        raise ValueError(f'Unexpected surrounding text: "{surrounding}"')
    ...
    with warnings.catch_warnings(record=False):
        if context.project.name == 'PermianProjectQ3_2022':
            warnings.simplefilter("ignore")
        context.queried_domain_objects = list(context.project.data_frames().find_by_display_name(query_name))
